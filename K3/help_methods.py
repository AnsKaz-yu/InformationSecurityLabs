...
def key_candidates_for_length_three_texts(
    ciphertext_1,
    ciphertext_2,
    ciphertext_3,
    key_length,
    ...
    text_3 = clean_text(ciphertext_3)
    ...
    for column_index in range(key_length):
        ...
        col_3 = text_3[column_index::key_length]

        shift_scores = []
        for shift in range(ALPHABET_SIZE):
            score = (
                column_decrypt_score(col_1, shift)
                + column_decrypt_score(col_2, shift)
                + column_decrypt_score(col_3, shift)
            )
...
def score_key_length_three_texts(ciphertext_1, ciphertext_2, ciphertext_3, key_length):
    best_candidate = key_candidates_for_length_three_texts(
        ciphertext_1,
        ciphertext_2,
        ciphertext_3,
        key_length,
...
def key_length_candidates_three_texts(
    ciphertext_1,
    ciphertext_2,
    ciphertext_3,
    max_key_length=20,
    ...
    for key_length in range(1, max_key_length + 1):
        scores[key_length] = score_key_length_three_texts(
            ciphertext_1,
            ciphertext_2,
            ciphertext_3,
            key_length,
    ...
    for key_length, score in ranked:
        ...
        if current_bucket is None or score_bucket == current_bucket:
            group.append((key_length, score))
            current_bucket = score_bucket if current_bucket is None else current_bucket
            continue

        filtered.extend(_remove_multiples_inside_group(group))
        group = [(key_length, score)]
        current_bucket = score_bucket
